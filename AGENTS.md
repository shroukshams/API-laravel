...
- `composer dev`: run the local server, queue listener, log tailing, and Vite dev server together.
- `composer test`: clear config and run the PHPUnit suite.
- `php artisan test --compact tests/Feature/AddContextTest.php`: run one focused test file.
- `npm run dev`: start Vite only.
- `npm run build`: build production frontend assets.
- `composer pint` or `vendor/bin/pint --dirty --format agent`: format PHP code with Laravel Pint.

## Coding Style & Naming Conventions

Follow existing Laravel conventions and PSR-4 autoloading (`App\\` maps to `app/`). Use 4-space indentation for PHP, explicit parameter and return types, and braces for every control structure. Prefer descriptive names such as `isRegisteredForDiscounts` over abbreviations. Use StudlyCase for classes, camelCase for methods and variables, snake_case for database columns, and plural snake_case table names. Reuse existing Laravel utilities, resources, middleware, and traits before introducing new abstractions.

## Testing Guidelines

This project uses PHPUnit 12. Place integration and HTTP behavior tests in `tests/Feature`; place isolated logic tests in `tests/Unit`. Name test classes after the behavior under test, for example `AddContextTest`. Every code change should include or update a relevant test when behavior changes. Run the narrowest useful test first, then broaden to `composer test` when the change affects shared behavior.

## Commit & Pull Request Guidelines

Recent commits use short imperative subjects, often with Conventional Commit prefixes such as `feat:` and `chore:`. Keep commits focused and explain the reason for the change. Pull requests should include a concise summary, verification evidence such as test commands, linked issues when applicable, and screenshots only for UI changes.

## Security & Configuration Tips

Do not commit secrets or local-only environment values. Keep `.env` derived from `.env.example`. Use Laravel configuration and named routes instead of hard-coded URLs, and prefer migrations or seeders over manual database edits.
